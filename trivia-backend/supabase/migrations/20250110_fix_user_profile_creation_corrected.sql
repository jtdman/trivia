-- Fix user profile creation workflow (CORRECTED VERSION)
...
-- Create function to get user email from auth.users (helper function)
...
BEGIN
  ...
  -- Update user to platform admin (or create profile if it doesn't exist)
...
COMMENT ON FUNCTION public.get_user_email(UUID) IS 'Helper function to get user email from auth.users';
