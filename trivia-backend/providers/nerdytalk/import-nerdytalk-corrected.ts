...
interface VenueCorrection {
  original_name: string
  correct_venue: NerdyTalkVenue
}

interface CorrectionData {
  corrections: VenueCorrection[]
  remove_venues: string[]
}

class CorrectedNerdyTalkImporter {
  private apiCallsToday = 0
  private nerdyTalkProviderId: string | null = null
  private corrections: CorrectionData | null = null

  async loadCorrections(): Promise<void> {
    try {
      const correctionsFile = await fs.readFile('./providers/nerdytalk/nerdytalk-venue-corrections.json', 'utf-8')
      this.corrections = JSON.parse(correctionsFile)
      console.log(`Loaded ${this.corrections?.corrections.length} venue corrections`)
    } catch (error) {
      console.warn('No corrections file found, proceeding without corrections')
      this.corrections = { corrections: [], remove_venues: [] }
    }
  }

  applyCorrections(venues: NerdyTalkVenue[]): NerdyTalkVenue[] {
    if (!this.corrections) return venues

    console.log('Applying venue corrections...')
    
    // Remove venues that should be excluded
    let correctedVenues = venues.filter(venue => 
      !this.corrections!.remove_venues.includes(venue.venue_name)
    )
    
    console.log(`Removed ${venues.length - correctedVenues.length} duplicate venues`)

    // Apply corrections
    correctedVenues = correctedVenues.map(venue => {
      const correction = this.corrections!.corrections.find(c => 
        c.original_name === venue.venue_name
      )
      
      if (correction) {
        console.log(`Applying correction: ${venue.venue_name} -> ${correction.correct_venue.venue_name}`)
        return correction.correct_venue
      }
  ...
  deduplicateVenues(venues: NerdyTalkVenue[]): NerdyTalkVenue[] {
    const seen = new Set<string>()
    const deduplicated = []
    
    for (const venue of venues) {
      const key = `${venue.venue_name}|${venue.address}`.toLowerCase()
      if (!seen.has(key)) {
        seen.add(key)
        deduplicated.push(venue)
      } else {
        console.log(`Removed duplicate: ${venue.venue_name}`)
      }
    }
    
    console.log(`Deduplication: ${venues.length} -> ${deduplicated.length} venues`)
    return deduplicated
  }
  ...
  async searchGooglePlaces(venueName: string, address: string): Promise<any> {
  ...
  async importFromFile(filePath: string): Promise<void> {
    console.log('Starting corrected NerdyTalk venue import...')
    ...
    try {
      ...
      console.log(`Found ${data.total_venues} venues before corrections`)

      // Apply corrections and deduplication
      let correctedVenues = this.applyCorrections(data.venues)
      correctedVenues = this.deduplicateVenues(correctedVenues)

      console.log(`Processing ${correctedVenues.length} corrected venues`)

      let processed = 0
      let successful = 0
      let failed = 0

      for (const venue of correctedVenues) {
        processed++
        ...
        try {
          ...
          if (processed % 5 === 0) {
            console.log(`Progress: ${processed}/${correctedVenues.length} venues processed`)
      ...
      console.log('\\nCorrected NerdyTalk import completed!')
      console.log(`Processed: ${processed}/${correctedVenues.length}`)
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  const importer = new CorrectedNerdyTalkImporter()
  importer.importFromFile(filePath)
    .then(() => process.exit(0))
    .catch((error) => {
      console.error('Corrected NerdyTalk import failed:', error)
      process.exit(1)
    })
}